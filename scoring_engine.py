...
def local_group_policy(vulnerability, name):
    if name == "Minimum Password Age":
        if 30 <= int(policy_settings_content.get('PASS_MIN_DAYS')) <= 60:
    ...
    if name == "Maximum Password Age":
        if 30 <= int(policy_settings_content.get('PASS_MAX_DAYS')) <= 60:
            record_hit('Maximum password age is set to 60-90.', vulnerability[1]['Points'])
        else:
            record_miss('Local Policy')
    if name == "Maximum Login Tries":
        if 5 <= int(policy_settings_content.get('LOGIN_RETRIES')) <= 10:
            record_hit('Maximum login tries is set to 5-10.', vulnerability[1]['Points'])
        else:
            record_miss('Local Policy')
    if name == "Lockout Duration":
        if 30 == int(policy_settings_content.get('LOGIN_TIMEOUT')):
            record_hit('Lockout duration is set to 30.', vulnerability[1]['Points'])
        else:
            record_miss('Local Policy')
            #get rid of the following
            #fix
    if name == "Lockout Reset Duration":
        temp = re.search(r"(?<=ResetLockoutCount = )\d+", common_password_settings_content)
        if temp != None:
            if 30 <= (int(temp).group(0)):
    ...
    if name == "Password History":
        temp = password_settings_content.get('remember')
        if temp != None and temp <= 5 :
            if 5 <= int(temp):
    ...
    with open('/etc/login.defs', 'r') as file:
        ...
        for line in lines:
            ...
            if not line.startswith('#') and line:
                k, v = line.split()
                login_defs_dict[k] =  v
    return login_defs_dict

def load_common_password_settings():
    passwordLines = ''
...
try:
    ...
    f = open('/etc/CYBERPATRIOT/scoring_engine.log', 'w')
    e = traceback.format_exc()
    f.write(str(e))
    f.close()
    messagebox.showerror('Crash Report', 'The scoring engine has stopped working, a log has been saved to ' + os.path.abspath('scoring_engine.log'))
...
while True:
    try:
        ...
        common_password_settings_content = load_common_password_settings()
